      ...
      >
       ...
       <div class="flex flex-col gap-1">
        <div class="p-4 flex items-start gap-1 flex-col">
          ...
          <UTextarea  @blur="onBlur(data.content)"
          :color="data.content.error.invalid ?'red' :  'white'"
            @input="onInput(data.content)" v-model="data.content.value" class="w-full h-auto" />
        </div>
        <p
...
const onSubmit = () =>{
  if(data.value.name.value.trim().length===0 || data.value.content.value.trim().length===0){
